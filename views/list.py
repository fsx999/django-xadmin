...
class ListAdminView(ModelAdminView):
    ...
    def get_context(self):
        ...
        model_fields = [(f, f.name in self.list_display, self.get_check_field_url(f))
                        for f in (list(self.opts.fields) + list(self.get_model_method_fields())) if f.name not in self.list_exclude]
    ...
    def result_item(self, obj, field_name, row):
        ...
        if (item.row['is_display_first'] and not self.list_display_links) \
            ...
            if self.list_display_links_details:
                item_res_uri = self.model_admin_url("detail", pk=getattr(obj, self.pk_attname))
                if item_res_uri:
                    edit_url = self.model_admin_url("change", pk=getattr(obj, self.pk_attname))
                ...
                item.wraps.append(u'<a href="%s">{}</a>' % url)
